...
// AF Enjoyer: Adding a new option here for twitch safety
class TwitchSafetyOption extends Option
{
	...
	{
		FlxG.save.data.twitchSafety = !FlxG.save.data.twitchSafety;
		
		if(FlxG.random.int(0, 100) < 20 && FlxG.save.data.twitchSafety)
			display = 'You\'re A Pussy';
		else
			display = updateDisplay();
		return true;
	}
	...
	{
		return  FlxG.save.data.twitchSafety ? 'Twitch Safety On' : "Twitch Safety Off";
	}
}

// AF Enjoyer: Adding another for people who suck (CLSL)
class YouSuckOption extends Option {
	public function new(desc:String) {
		super();
		description = desc;
	}
	public override function press():Bool {
		FlxG.save.data.youSuck = !FlxG.save.data.youSuck;
		FlxG.save.data.ghost = FlxG.save.data.youSuck;

		display = updateDisplay();
		return true;
	}
	private override function updateDisplay():String {
		return FlxG.save.data.youSuck ? "You Suck Mode On" : 'You Suck Mode Off';
	}

}

// AF Enjoyer: Adding an option to reset viewed clips
class ResetViewedClipsOption extends Option
{
	...
	public override function press():Bool
	{
		...
		ClipsGalleryState.resetViewedClips();
		trace('Viewed Data Wiped');
	...
	private override function updateDisplay():String
	{
		return confirm ? "Confirm Viewed Data Reset" : "Reset Viewed Data";
	}
}
...
class GhostTapOption extends Option
{
	...
	public override function press():Bool {
		// AF Enjoyer: Changing this so it literally cant be turned on
	...
	private override function updateDisplay():String {
		return hasPressed ? "Nice Try" : "No Ghost Tapping";
	}
}
...
class ResetSettings extends Option
{
	...
	public override function press():Bool
	{
		...
		// AF Enjoyer: Adding my little option here
		FlxG.save.data.hasPlayed = null;
		FlxG.save.data.songsUnlocked = null;
		FlxG.save.data.twitchSafety = null;
		FlxG.save.data.hasRevivedBefore = null;
