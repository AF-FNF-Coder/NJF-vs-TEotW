...
class WebmPlayer extends Bitmap
{
	...
	// AF Enjoyer: Gotta add these variables so that io and soundEnabled are saved for the bitmap thingy

	public var ioSaved:WebmIo;
	public var soundEnabledSaved:Bool;
	...
	{
		// AF Enjoyer: Saving these
		ioSaved = io;
		soundEnabledSaved = soundEnabled;
	...
	function stepVideoFrame() {
		if (playing) {
			...
			//trace('elapsed time: ' + elapsedTime + '; lastDecodedVideoFrame: ' + lastDecodedVideoFrame);
	...
	function decodeVideoFrame(time:Float, data:BytesData) {
		if (playing) {
			...
			// AF Enjoyer: Restarting caused the game to crash because bitmapData was null, and the fuck function is what makes it not null, so...
				if(bitmapData == null) {
					trace('Had to use the saved data: ' + ioSaved);
					fuck(ioSaved, soundEnabledSaved);
				}
				// AF Enjoyer: Worked for 22 hours for this one line of code. Basically, videos will play according to their elapsedTime. They wont go too fast or too slow, causing them
				//	to be perfectly synced with the audio, always.
				if((lastDecodedVideoFrame > getElapsedTime()))
