...
/*
	...
	public static function playRandomAFSong() {
		trace('FlxG.save.data.hasPlayed: ' + FlxG.save.data.hasPlayed);
		var possibleSongsArray:Array<String>;
			if(FlxG.save.data.hasPlayed) {
				possibleSongsArray = ['Early Days Era - Wall', 'Groyper Wars Era - Heated Gamer Moment', 'WBS Era -  G Nick', 'WBS Era - Higher Power'/*, 'AFPAC I Era - Inevitable'*/];
			} else {
				// AF Enjoyer: I think this is the best one, so everyone will get it on their first run
				possibleSongsArray = ['WBS Era -  G Nick'];
				new FlxTimer().start(5, function(tmr:FlxTimer) {
					FlxG.save.data.hasPlayed = true;
				});
			}
			trace('possibleSongsArray: ' + possibleSongsArray);
			trace('FlxG.random.getObject(possibleSongsArray): ' + FlxG.random.getObject(possibleSongsArray));
			FlxG.sound.playMusic(Paths.music(FlxG.random.getObject(possibleSongsArray)), 0);
	...
	override function update(elapsed:Float)
	{
		...
		// AF Enjoyer: Trying to change this so right after the Credo song, it goes into the menu
		if (!transitioning && skippedIntro)
			...
			*/
			...
			// AF Enjoyer: This is for outdated Kade Engine warnings. It is probably a good idea to disable this in the finished version of the mod
			/*
	...
	function skipIntro():Void
		...
		/*
		// AF Enjoyer: Im just gonna re-do the functions with my code
		override public function create():Void
		{
			#if polymod
			polymod.Polymod.init({modRoot: "mods", dirs: ['introMod']});
		...
		function startIntro()
			{
				...
				if (initialized)
					...
					if(FlxG.save.data.hasPlayed == true) {
						...
						possibleSongsArray = ['AFPAC I Era - Inevitable'];
					}
					FlxG.sound.playMusic(Paths.music(FlxG.random.getObject(possibleSongsArray)), 0);
